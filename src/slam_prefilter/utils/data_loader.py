...
def create_default_sequence(scene_root: Optional[Path] = None) -> "RgbFrameSequence":
    """Convenience helper for quick local testing with an explicit scene root."""
    if scene_root is None:
        raise ValueError("scene_root must be provided explicitly")
    sequence, _ = RgbFrameSequence.from_scene_root(scene_root)
    return sequence
